...
const rooms = {
  ...
  'ethereal-nexus': {
    ...
    name: 'ethereal-nexus',
    description:
      'You look around to see space itself, you see stars, planets and galaxies all unfamilar to you. Looking down you see your standing on a strange path that allmst seems to be made of glowing water.',
    weight: 0.1,
  },
  'bolted-passage': {
...
const generateRooms = (currentRooms: Room<(typeof roomNames)[number]>[]) => {
  const roomList = Object.values(rooms);
  const blockedRooms = new Set<string>();
  const favouredRooms = new Set<string>();
  const currentRoomNames = currentRooms.map((room) => room.name);
  ...
  currentRoomsFirst.forEach((room) => {
    ...
    const weight = room.weight - negativeWeight;
